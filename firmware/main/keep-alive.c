/* Keep Alive engine for wss server example
   ...
   From: esp-idf/examples/protocols/https_server/wss_server
...
static uint64_t
get_max_delay(wss_keep_alive_t h)
{
...
static bool
update_client(wss_keep_alive_t h, int sockfd, uint64_t timestamp)
{
  for (int i = 0; i < h->max_clients; ++i) {
...
static bool
remove_client(wss_keep_alive_t h, int sockfd)
{
  for (int i = 0; i < h->max_clients; ++i) {
    if (h->clients[i].type == CLIENT_ACTIVE && h->clients[i].fd == sockfd) {
...
static bool
add_new_client(wss_keep_alive_t h, int sockfd)
{
  for (int i = 0; i < h->max_clients; ++i) {
        ...
        if (keep_alive_storage->clients[i].type == CLIENT_ACTIVE) {
          if (keep_alive_storage->clients[i].last_seen + keep_alive_storage->keep_alive_period_ms <= _tick_get_ms()) {
            ...
            if (keep_alive_storage->clients[i].last_seen + keep_alive_storage->not_alive_after_ms <= _tick_get_ms()) {
              ESP_LOGD(TAG, "Client (fd=%d) not alive!", keep_alive_storage->clients[i].fd);
...
wss_keep_alive_t
wss_keep_alive_start(wss_keep_alive_config_t *config)
{
  ...
  wss_keep_alive_t keep_alive_storage =
    calloc(1, sizeof(wss_keep_alive_storage_t) + client_list_size * sizeof(client_fd_action_t));
  ...
  if (xTaskCreate(keep_alive_task,
                  "keep_alive_task",
...
void
wss_keep_alive_stop(wss_keep_alive_t h)
{
...
esp_err_t
wss_keep_alive_add_client(wss_keep_alive_t h, int fd)
{
...
esp_err_t
wss_keep_alive_remove_client(wss_keep_alive_t h, int fd)
{
...
esp_err_t
wss_keep_alive_client_is_active(wss_keep_alive_t h, int fd)
{
...
void
wss_keep_alive_set_user_ctx(wss_keep_alive_t h, void *ctx)
{
  h->user_ctx = ctx;
}
...
void *
wss_keep_alive_get_user_ctx(wss_keep_alive_t h)
{
  return h->user_ctx;
}
