...
/*
 * WebSocket smoke test for the ESP32 firmware interface.
 *
 * Usage:
 *   WS_HOST=192.168.1.100 node test_websocket.js
 *   WS_HOST=192.168.1.100 WS_PORT=80 node test_websocket.js
 *
 * Optional if your Node.js doesn't expose global WebSocket:
 *   npm install ws
 */
...
if (!Number.isInteger(WS_PORT) || WS_PORT <= 0) {
  console.error("WS_PORT must be a positive integer");
  process.exit(1);
}
...
async function getWebSocketCtor() {
  ...
  try {
    ...
    console.error("Use Node.js with global WebSocket or run: npm install ws");
    process.exit(1);
  }
}
...
(async () => {
  ...
  console.log(`Testing websocket interface on ${WS_HOST}:${WS_PORT}`);
  await testHandshakePath(WS, "/ws1");
  await testHandshakePath(WS, "/ws2");
  await testToggleBroadcastDirection(WS, "/ws1", "/ws2");
  await testToggleBroadcastDirection(WS, "/ws2", "/ws1");
